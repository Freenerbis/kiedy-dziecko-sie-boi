...
export const IPhoneMockup: React.FC<IPhoneMockupProps> = ({
  ...
  bezel, radius, shadow = true, screenBg = '#000', wallpaper, wallpaperFit = 'cover', wallpaperPosition = 'center',
  showDynamicIsland, showNotch, islandWidth, islandHeight, islandRadius, notchWidth, notchHeight, notchRadius,
  safeArea = true, safeAreaOverrides, showHomeIndicator = true, innerShadow = true,
  ...
  const outerShadow = typeof shadow === 'string' ? shadow : shadow ? `0 24px 60px rgba(0,0,0,0.5), 0 4px 12px rgba(0,0,0,0.3)` : 'none';
  const innerShadowCss = innerShadow ? 'inset 0 0 0 1px rgba(255,255,255,0.03), inset 0 10px 20px rgba(0,0,0,0.35)' : 'none';
  ...
  return (
    <div className={className} style={{ boxSizing: 'border-box', display: 'inline-block', transform: `scale(${scale})`, transformOrigin: 'top left', ...style }}>
      <div style={{ width: outerWidth, height: outerHeight, borderRadius: outerRadius, background: frameGradient, padding: resolvedBezel, boxSizing: 'border-box', boxShadow: outerShadow, position: 'relative', overflow: 'hidden', ...frameStyle }}>
        <div style={{ width: '100%', height: '100%', borderRadius: resolvedRadius, position: 'relative', overflow: 'hidden', background: screenBg, boxShadow: innerShadowCss, ...screenStyle }}>
          ...
          {showHomeIndicator && (
            <div aria-hidden style={{ position: 'absolute', bottom: 8, left: '50%', transform: 'translateX(-50%)', width: Math.round(screenWidth * 0.34), maxWidth: 140, height: 5, borderRadius: 3, background: 'linear-gradient(180deg, rgba(255,255,255,0.7), rgba(255,255,255,0.35))', opacity: 0.9, zIndex: 3 }} />
          )}
        </div>
      </div>
    </div>
